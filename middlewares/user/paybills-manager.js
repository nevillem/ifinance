$(document).ready(function(){
    $('#saccoPaybillsForm').validate({
      ...
      messages: {
        ...
        expenseaccount: {
            required: "please choose expense account!"
        },
        amount: {
    ...
    $('#managerbillpaymentform').validate({
      rules: {
        accounttospendfrom: {
          required: true,
        },
    ...
    $("#vendorbilldata").on("change", function(){
                ...
                async function get_billinginfo() {
                    if(vendorbilldata){
                        ...
                        $.ajax({
                            url: base+"managersaccovendors/"+bill,
                            headers: {
                                'Authorization': localStorage.token,
                                'Content-Type': 'application/json'
                            },
                            ...
                            success: function(response) {
                               var nums = response.data.rows_returned;
                                ...
                                bills_ += '<td class="fw-60 text-center"><input id="chk" name="checkbox1" type="checkbox"  value="'+bills[b].id+'"></td>';           
              ...
              $('#managerbillpaymentform').submit(function(event) {
                    event.preventDefault();
                    if ($('#managerbillpaymentform').valid()) {
                        managerbillpaymentform=$(this);
    
    
                    var formData=JSON.stringify(managerbillpaymentform.serializeObject());
                      //cancelIdleCallback
                      ...
                      // submit form data to api
                        async function payBill() {
                            ...
                            $.ajax({
                                ...
                                data: formData,
                                success: function(response) {
                                    managerbillpaymentform[0].reset();
                                    var icon = 'success';
