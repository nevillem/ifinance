$(document).ready(function(){
                  ...
                  $.ajax({
                      ...
                      success: function(response) {
                          ...
                          $('#account_from').select2({
                                    theme: 'bootstrap5',
                                    width: 'resolve',
                                    dropdownParent: $("#transferToGropForm")
                                });
                          // $('#account').select2({
                          //           theme: 'bootstrap5',
                          //           width: 'resolve',
              ...
              $.ajax({
                  ...
                  success: function(response) {
                    console.log(response);
                      nums = response.data.rows_returned;
                     if (nums !=0) {
                       $('#member_account').html('<option value="">Select Client Receiving group</option>');
                       for (var i = 0; i < nums; i++) {
                         var accounts = '';
                         accounts += '<option value=' + response.data.accounts[i].id + '>' + response.data.accounts[i].code + ' | '+response.data.accounts[i].account +'</option>';
                         $('#member_account').append(accounts);
                       }
                     }
                  ...
                  error: function(xhr){
                          if (xhr.status == '401') {
                            ...
                            $('.member_account2').html('<option value="">No group accounts found</option>');
          ...
          getAccountsView()
           ...
           $('.member_account2').html('<option value="">Select group Receiving first</option>');
         }
         });
        ...
        $('#previous').click(function(){
          ...
          current_fs.hide();
              ...
              $('#transferToGropForm').submit(function(event) {
                    event.preventDefault();
                    if ($('#transferToGropForm').valid()) {
                      transferToGropForm=$(this);

                    var formData=JSON.stringify(transferToGropForm.serializeObject());
                    console.log(formData);
                        //   cancelIdleCallback
                        clientToGroups();
                        async function clientToGroups() {
                            // start ajax loader
                            $.ajax({
                                url: base+"withdrawclienttogroup",
                                headers: {
                                    'Authorization': localStorage.token,
                                    'Content-Type': 'application/json'
                                },
                ...
                // get client to client transfers
                   get_client2group();
                   async function get_client2group() {
                       $.ajax({
                           url: base+"withdrawclienttogroup",
                           headers: {
                               'Authorization': localStorage.token,
                           ...
                           success: function(response) {
                               ...
                               for (var i = 0; i < transfer_from; i++) {
                                   no++
                                   var client2group_transfers = '';
                                   client2group_transfers += '<tr>';
                                   client2group_transfers += '<td> </td>';
                                   client2group_transfers += '<td>' +no+ '</td>';
                                   client2group_transfers += '<td>' +response.data.transactions.clientstransferfrom[i].firstname+ ' '+response.data.transactions.clientstransferfrom[i].lastname+ '</td>';
                                   client2group_transfers += '<td>' +response.data.transactions.clientstransferfrom[i].accountname+ '</td>';
                                   client2group_transfers += '<td>' +response.data.transactions.clientstransferfrom[i].amount+ '</td>';

                                   // clientstransferto

                                   client2group_transfers += '<td>' +response.data.transactions.clientstransferto[i].firstname+ ' '+response.data.transactions.clientstransferfrom[i].lastname+ '</td>';
                                   client2group_transfers += '<td>' +response.data.transactions.clientstransferto[i].accountname+ '</td>';
                                   client2group_transfers += '<td>' +response.data.transactions.clientstransferto[i].method+ '</td>';
                                   client2group_transfers += '<td>' +response.data.transactions.clientstransferto[i].timestamp+ '</td>';
                                   client2group_transfers += '<td>' +response.data.transactions.clientstransferto[i].transactionID+ '</td>';
                                   client2group_transfers += '<td>' +response.data.transactions.clientstransferto[i].status+ '</td>';

                                   client2group_transfers += '</tr>';
                                   $("#client_2_grouptransfers").append(client2group_transfers);
                               }

                               $('#dataTables-client_2_grouptransfers').DataTable({
                                   responsive: false,
                                   processing: true,
                                   serverSide: false,
                                   retrieve: true,
