...
$(document).ready(function() {
    ...
    // sacco default settings
    $('#saccoDefaultsettingForm').validate({
        rules: {
          currency: {
            required: true,
          },
        ...
        messages: {
          currency:{
                  required: "please specify your currency!",
                },
          shares_account: {
              required: "please specify shares account!"
          },
          loan_interest_account: {
              required: "please specify loan interest account!"
          },
          numberofloanstoguarantee: {
              required: "this is a required input please!"
          },
          withdraws_account: {
              required: "specify withdraw account please!"
          },
          },
        errorElement: 'span',
    ...
    async function get_account() {
        $.ajax({
            ...
            success: function(response) {
                ...
                for (var i = 0; i < nums; i++) {
                    var accoun_t = '';
                    accoun_t += '<option value="' + response.data.account[i].account + '">'+ response.data.account[i].account +' | '+ response.data.account[i].account_group +' | '+ response.data.account[i].status +'</option>';
                    $('#shares_account').append(accoun_t);
                    $('#loan_interest_account').append(accoun_t);
                    $('#withdraws_account').append(accoun_t);
                }
            
                $('#shares_account').select2({
                    theme: 'bootstrap5',
                    width: 'resolve',
                });

                $('#loan_interest_account').select2({
                    theme: 'bootstrap5',
                    width: 'resolve',
                });

                $('#withdraws_account').select2({
                    theme: 'bootstrap5',
                    width: 'resolve',
                });
            },
    ...
    $('#saccoDefaultsettingForm').submit(function(event) {
        event.preventDefault();
        if ($('#saccoDefaultsettingForm').valid()) {
            var saccoDefaultsettingForm = $(this);
            var form_data = JSON.stringify(saccoDefaultsettingForm.serializeObject());
            updateDefaultsettings();
            // submit form data to api
            async function updateDefaultsettings() {
                // start ajax loader
                $.ajax({
                    url: base + "systemdefault",
                    headers: {
                        'Authorization': localStorage.token,
                        'Content-Type': 'application/json'
                    },
                    ...
                    data: form_data,
                    success: function(response) {
                        saccoDefaultsettingForm[0].reset();
                        var icon = 'success';
                    ...
                    error: function(xhr, status, error) {
                        if (xhr.status === 401) {
                            authchecker(updateDefaultsettings);
                        } else {
                            var icon = 'warning'
                            var message = xhr.responseJSON.messages
                            sweetalert(icon, message)
