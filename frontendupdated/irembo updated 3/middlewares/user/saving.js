...
async function getBankAccounts() {
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var i = 0; i < nums; i++) {
                var bankaccount = '';
                bankaccount += '<option value="' + response.data.bankaccounts[i].account + '">' + response.data.bankaccounts[i].account +'</option>';
                $('#bank').append(bankaccount);
            }
            $('#bank').select2({
                theme: 'bootstrap5',
                width: 'resolve',
...
async function getPaymentMethods() {
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var i = 0; i < nums; i++) {
                var modeofp = '';
                modeofp += '<option value=' + response.data.paymentmethods[i].paymentmethod + '>' + response.data.paymentmethods[i].paymentmethod + '</option>';
                $('.modeofpayment').append(modeofp);
            }
