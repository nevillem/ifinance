...
class SaccoException extends Exception { }
...
class Sacco {
  ...
  // constructor to create the user object with the instance variables already set
	public function __construct($id, $name, $username, $password, $contact, $status, $role, $branch,$sacco) {
		$this->setID($id);
		$this->setName($name);
  ...
  public function setBranch($branch) {
    ...
    if(!is_numeric($branch) || $branch <= 0 || $branch > 9223372036854775807):
      throw new UserException("User branch error");
    endif;
    $this->_branch = $branch;
  }
