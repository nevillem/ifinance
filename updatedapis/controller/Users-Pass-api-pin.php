    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST'):
      try {
        ...
        if(!isset($jsonData->oldpass)):
          ...
          $query = $readDB->prepare('select user_pincode, user_email,user_contact from users where user_id = :id');
          ...
          $contact = $row['user_contact'];
           if (!password_verify($oldpass, $row['user_pincode'])):
             $response = new Response();
             $response->setHttpStatusCode(406);
             $response->setSuccess(false);
             $response->addMessage("incorrect old pin");
             $response->send();
             exit;
            endif;
            $newpin = rand(1001,9999);
            $pass_hash = password_hash($newpin, PASSWORD_DEFAULT);
            $query = $writeDB->prepare('update users set user_pincode = :pin where user_id = :id');
            $query->bindParam(':pin', $pass_hash, PDO::PARAM_STR);
            $query->bindParam(':id', $returned_id, PDO::PARAM_INT);
            $query->execute();
            $rowCount = $query->rowCount();
            ...
            $message = "your new irembo finance pin code is ".$newpin;
            // insert sms into the database
            insertSMSDB($writeDB, $message, $contact, $returned_saccoid);
            // insert email into the database
            insertEMAILDB($writeDB, $message, $email, $returned_saccoid);
            $response = new Response();
            $response->setHttpStatusCode(201);
            $response->setSuccess(true);
            $response->addMessage("Pin updated success");
            $response->send();
      } catch (PDOException $ex) {
        $response = new Response();
        $response->setHttpStatusCode(500);
        $response->setSuccess(false);
        $response->addMessage("interenal server error".$ex);
        $response->send();
        exit;
      }
