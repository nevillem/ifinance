...
try {
  ...
  if (array_key_exists('vendorsid', $_GET)) {
    ...
    elseif($_SERVER['REQUEST_METHOD'] === 'DELETE'){
      try {
        // ADD AUTH TO QUERY
        $query = $writeDB->prepare('DELETE from sacco_vendors where vendors_id=:vendors_id AND vendors_saccoid = :saccoid');
        $query->bindParam(':vendors_id', $vendorsid, PDO::PARAM_STR);
        ...
        if($rowCount === 0):
                ...
                $response->setHttpStatusCode(404);
                $response->setSuccess(false);
                $response->addMessage("sacco vendor not found");
                $response->send();
                exit;
        else:
                $response = new Response();
                $response->setHttpStatusCode(200);
                $response->setSuccess(true);
                $response->addMessage("sacco vendor deleted");
                $response->send();
                exit;
      endif;
      }
        ...
        $response->setHttpStatusCode(500);
        $response->setSuccess(false);
        $response->addMessage("Failed to delete sacco vendor - Attached Info");
        $response->send();
        exit;
      }
    ...
    try{
      ...
      if(isset($jsonData->email)):
        ...
        $emailupdate = true;
        // add address field to query field string
        $queryFields .= "vendors_email  = :email, ";
      endif;
      if(isset($jsonData->companyname)):
        // set Address field updated to true
        $companyupdate = true;
        // add address field to query field string
        $queryFields .= "company_name  = :companyname, ";
      endif;
      if(isset($jsonData->contact)):
        // set Address field updated to true
        $contactupdate = true;
        // add address field to query field string
        $queryFields .= "vendors_contact  = :contact, ";
      endif;
      if(isset($jsonData->address)):
        // set Address field updated to true
        $addressupdate = true;
        // add address field to query field string
        $queryFields .= "vendors_address  = :address, ";
      endif;
      // remove the right hand comma and trailing space
      $queryFields = rtrim($queryFields, ", ");

      // check if any branch fields supplied in JSON
      if($firstnameupdate === false && $lastnameupdate === false && $emailupdate === false
      && $companyupdate === false && $contactupdate === false && $addressupdate === false):
        $response = new Response();
        $response->setHttpStatusCode(400);
        $response->setSuccess(false);
      ...
      $query = $writeDB->prepare('SELECT * from sacco_vendors where vendors_id=:vendors_id
      ...
      if($rowCount === 0):
        ...
        $response->setHttpStatusCode(404);
        $response->setSuccess(false);
        $response->addMessage("No sacco vendor found to update");
        $response->send();
        exit;
      endif;
      // create the query string including any query fields
      $queryString = "UPDATE sacco_vendors set ".$queryFields." where vendors_id  = :id";
      // prepare the query
      $query = $writeDB->prepare($queryString);

      // if name has been provided
      if($firstnameupdate === true):
      ...
      // if name has been provided
      if($companyupdate === true):
        // bind the parameter of the new value from the object to the query (prevents SQL injection)
        $query->bindParam(':companyname', $jsonData->companyname, PDO::PARAM_STR);
      endif;
      // if name has been provided
      ...
      $query->bindParam(':id', $vendorsid, PDO::PARAM_STR);
      $query->execute();
      if($rowCount === 0):
        // set up response for unsuccessful return
        $response = new Response();
      ...
      $response->setHttpStatusCode(200);
      $response->setSuccess(true);
      $response->addMessage("vendor has been updated");
      $response->setData($returnData);
      $response->send();
      exit;
    ...
    // if error with sql query return a json error
      ...
      $response->setHttpStatusCode(500);
      $response->setSuccess(false);
      $response->addMessage("Failed to update vendor - check your data for errors" . $ex);
      $response->send();
      exit;
    }
    }
