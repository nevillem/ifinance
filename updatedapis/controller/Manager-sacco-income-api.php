...
try {
  ...
  if (array_key_exists('incomeid', $_GET)) {
    ...
    elseif($_SERVER['REQUEST_METHOD'] === 'DELETE'){
      try {
        // ADD AUTH TO QUERY
        $incomequery = $writeDB->prepare('DELETE from income_tb where where  income_sacco_id = :saccoid
          AND income_id  =:incomeid');
        $incomequery->bindParam(':incomeid', $incomeid, PDO::PARAM_STR);
        $incomequery->bindParam(':saccoid', $returned_saccoid, PDO::PARAM_STR);
        $incomequery->execute();
        ...
        if($rowCount === 0):
                ...
                $response->setHttpStatusCode(404);
                $response->setSuccess(false);
                $response->addMessage("sacco income not found");
                $response->send();
                exit;
        else:
                $response = new Response();
                $response->setHttpStatusCode(200);
                $response->setSuccess(true);
                $response->addMessage("sacco income deleted");
                $response->send();
                exit;
      endif;
      }
        ...
        $response->setHttpStatusCode(500);
        $response->setSuccess(false);
        $response->addMessage("Failed to delete sacco income - Attached Info");
        $response->send();
        exit;
      }
