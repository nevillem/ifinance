       ...
       elseif ($loan_rate_type ==='straight line') {
         ...
         for ($i=1; $i <= $loantenure; $i++) {
           ...
           "principalamountpaid"=>number_format($pp),
            ...
            "loan_balance"=>number_format(abs($amount)));
           $scheduleArray[]=$schedule;
         }
       }
       break;
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'PATCH') {
    ...
    try {
      ...
      if($rowCount === 0) {
        ...
        $response->setHttpStatusCode(404);
        $response->setSuccess(false);
        $response->addMessage("loan application not found for this group");
        $response->send();
        exit;
      }
      ...
      $row = $query->fetch(PDO::FETCH_ASSOC);
        ...
        $row = $query->fetch(PDO::FETCH_ASSOC);
         ...
         $message = "Hello, your group loan application of UGX ".$newAmount." has been reviewed and processed, you will be notified once the loan is approved. Loan ID ".$loan_app_number. ". Date: ".$date;
