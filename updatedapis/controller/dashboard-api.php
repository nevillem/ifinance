...
if($_SERVER['REQUEST_METHOD'] === 'GET'):
  ...
  try {
    // create db query
    // ADD AUTH TO QUERY
    $query = $readDB->prepare('select * from branches where saccos_sacco_id = :saccoid');
    ...
    $returnData['members'] = $rowCount;

    // ADD AUTH TO QUERY
    $query = $readDB->prepare('select * from users where saccos_sacco_id = :saccoid');
    $query->bindParam(':saccoid', $returned_saccoid, PDO::PARAM_INT);
    $query->execute();
