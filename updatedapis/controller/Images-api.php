...
function getImageRoute($readDB, $memberid, $imageid, $returned_saccoid) {
  // attempt to query the database
  try {
    // create db query
    // ADD AUTH TO QUERY
    $query = $readDB->prepare('SELECT tblimages.id, tblimages.title, tblimages.filename, tblimages.mimetype, tblimages.members_member_id from tblimages, members where tblimages.id = :imageid and members.member_id = :memberid and members.saccos_sacco_id = :saccoid and tblimages.members_member_id = members.member_id');
    ...
    while($row = $query->fetch(PDO::FETCH_ASSOC)) {
      // create new member object for each row
      $image = new Image($row['id'], $row['title'], $row['filename'], $row['mimetype'], $row['members_member_id']);
    }
...
function getImageAttributesRoute($readDB, $memberid, $imageid, $returned_saccoid) {
  // attempt to query the database
  try {
    // create db query
    // ADD AUTH TO QUERY
...
function updateImageAttributesRoute($writeDB, $memberid, $imageid, $returned_saccoid) {
  ...
  try {
    ...
    $query = $writeDB->prepare('SELECT tblimages.id, tblimages.title, tblimages.filename, tblimages.mimetype, tblimages.members_member_id from tblimages, members where tblimages.id = :imageid and tblimages.members_member_id = :memberid and tblimages.members_member_id = members.member_id and tblmembers.saccos_sacco_id = :saccoid');
    $query->bindParam(':imageid', $imageid, PDO::PARAM_INT);
    $query->bindParam(':memberid', $memberid, PDO::PARAM_INT);
    $query->bindParam(':saccoid', $returned_saccoid, PDO::PARAM_INT);
    $query->execute();
    ...
    // make sure that the image exists for a given member and image id
    if($rowCount === 0) {
    ...
    $queryString = "update tblimages inner join members on tblimages.members_member_id = members.member_id set ".$queryFields." where tblimages.id = :imageid and tblimages.members_member_id = members.member_id and tblimages.members_member_id = :memberid and members.saccos_sacco_id = :saccoid";
    // prepare the query
    ...
    $query->bindParam(':imageid', $imageid, PDO::PARAM_INT);
    // bind the member id provided in the query string
    $query->bindParam(':memberid', $memberid, PDO::PARAM_INT);
    ...
    $query = $writeDB->prepare('SELECT tblimages.id, tblimages.title, tblimages.filename, tblimages.mimetype, tblimages.members_member_id from tblimages, members where tblimages.id = :imageid and members.member_id = :memberid and members.saccos_sacco_id = :saccoid and tblimages.members_member_id = members.member_id');
    $query->bindParam(':imageid', $imageid, PDO::PARAM_INT);
    $query->bindParam(':memberid', $memberid, PDO::PARAM_INT);
    $query->bindParam(':saccoid', $returned_saccoid, PDO::PARAM_INT);
    $query->execute();
...
function deleteImageRoute($writeDB, $memberid, $imageid, $returned_saccoid) {
  try {
    ...
    $query = $writeDB->prepare('delete tblimages from tblimages, tblmembers where tblimages.id = :imageid and members.members_member_id = :memberid and tblimages.members_member_id = members.members_member_id and members.saccos_sacco_id = :saccoid');
    $query->bindParam(':imageid', $imageid, PDO::PARAM_INT);
    $query->bindParam(':memberid', $memberid, PDO::PARAM_INT);
    $query->bindParam(':saccoid', $returned_saccoid, PDO::PARAM_INT);
    $query->execute();
...
function uploadImageRoute($readDB, $writeDB, $memberid, $returned_saccoid) {
  ...
  try {
    ...
    // ADD AUTH TO QUERY
    $query = $readDB->prepare('SELECT member_id from members where member_id = :memberid and saccos_sacco_id = :saccoid');
    $query->bindParam(':memberid', $memberid, PDO::PARAM_INT);
    $query->bindParam(':saccoid', $returned_saccoid, PDO::PARAM_INT);
    $query->execute();
    ...
    // if no member found then send response back
    if($rowCount === 0) {
      // send json error response using function
    ...
    $image = new Image(null, $jsonImageAttributes->title, $jsonImageAttributes->filename.$fileExtension,
    $imageFileDetails['mime'], $memberid);
    ...
    // check if filename already exits for any image for this member
    $query = $readDB->prepare('SELECT tblimages.id from tblimages, members where tblimages.members_member_id = members.member_id and members.member_id = :memberid and members.saccos_sacco_id = :saccoid and tblimages.filename = :filename');
    $query->bindParam(':memberid', $memberid, PDO::PARAM_INT);
    $query->bindParam(':saccoid', $returned_saccoid, PDO::PARAM_INT);
    ...
    if($rowCount !== 0) {
      ...
      // send json error response using function
      sendResponse(409, false, "A file with that filename already exists for this member - try a different filename");
    }
    ...
    $writeDB->beginTransaction();
    $query = $writeDB->prepare('insert into tblimages (title, filename, mimetype, members_member_id,saccos_sacco_id) values (:title, :filename, :mimetype, :memberid, :saccoid)');
    ...
    // get last member id so we can return the Image attributes in the json
    $lastImageID = $writeDB->lastInsertId();

    $query = $writeDB->prepare('SELECT tblimages.id, tblimages.title, tblimages.filename, tblimages.mimetype, tblimages.members_member_id from tblimages, members where tblimages.id = :imageid and tblimages.members_member_id = :memberid and members.saccos_sacco_id = :saccoid and tblimages.members_member_id = members.member_id');
    ...
    // send json error response using function
    sendResponse(500, false, "Failed to upload image".$ex);
    exit;
  }
  // if error with image deletion return as json error
  catch(ImageException $ex) {
...
$returned_saccoid = checkAuthStatusAndReturnUserID($writeDB);
...
// check if imageid, member id and attributes is in the url e.g. /members/1/images/5/attributes - get or update image attributes
if(array_key_exists("memberid",$_GET) && array_key_exists("imageid",$_GET) && array_key_exists("attributes",$_GET)) {
  ...
  //check to see if image id/memberid in query string is not empty and is number, if not return json error
  if($imageid == '' || !is_numeric($imageid) || $memberid == '' || !is_numeric($memberid)) {
    // send json error response using function
    sendResponse(400, false, "Image ID or member ID cannot be blank or must be numeric");
  }

  // if request is a GET, e.g. get image attribute - /v1/members/1/images/5/attributes
  if($_SERVER['REQUEST_METHOD'] === 'GET') {

    // call getImageAttributesRoute function - passing in memberid and imageid
    getImageAttributesRoute($readDB, $memberid, $imageid, $returned_saccoid);
  }
  // if request is a PATCH, e.g. update only image attribute and not image file - /v1/members/1/images/5/attributes
  elseif($_SERVER['REQUEST_METHOD'] === 'PATCH') {

    // call updateImageAttributesRoute function - passing in memberid and imageid
    updateImageAttributesRoute($writeDB, $memberid, $imageid, $returned_saccoid);
  }
...
// check if imageid and member id is in the url e.g. /members/1/images/5 - get image
elseif(array_key_exists("memberid",$_GET) && array_key_exists("imageid",$_GET)) {
  ...
  // if request is a GET, e.g. get image - /v1/members/1/images/5
  if($_SERVER['REQUEST_METHOD'] === 'GET') {

    // call getImageRoute function - passing in memberid and imageid
    getImageRoute($readDB, $memberid, $imageid, $returned_saccoid);
  }
  // if requets is a DELETE, e.g. delete member image
  elseif($_SERVER['REQUEST_METHOD'] === 'DELETE') {

    // call deleteImageRoute function - passing in memberid and imageid
    deleteImageRoute($writeDB, $memberid, $imageid, $returned_saccoid);
  }
...
// handle creating new member image
elseif(array_key_exists("memberid",$_GET) && !array_key_exists("imageid",$_GET)) {

  // get member id from query string
  $memberid = $_GET['memberid'];

  //check to see if memberid in query string is not empty and is number, if not return json error
  if($memberid == '' || !is_numeric($memberid)) {
    // send json error response using function
    sendResponse(400, false, "member ID cannot be blank or must be numeric");
  }

  // if request is a POST e.g. create member image
  if($_SERVER['REQUEST_METHOD'] === 'POST') {

    // call uploadImageRoute function

    uploadImageRoute($readDB, $writeDB, $memberid, $returned_saccoid);
