...
if (array_key_exists("loanid",$_GET)) {
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'PATCH'){
          try {
            ...
            $returnData = array();
            $returnData['rows_returned'] = $rowCount;
            $returnData['loanapplication'] = $loanArray;
    ...
    try {
      ...
      if ($status === 'approved') {
       ...
       $row = $query->fetch(PDO::FETCH_ASSOC);
        ...
        $message = "Hello, your loan application of UGX ".$newAmount." has been approved, you will be notified once the loan is disbursed. Loan ID ".$loan_app_number. ". Date: ".$date;
    ...
    if ($status === 'rejected') {
     ...
     $row = $query->fetch(PDO::FETCH_ASSOC);
      ...
      $message = "Hello, your loan application of UGX ".$newAmount." has been rejected. Loan ID ".$loan_app_number. ". Date: ".$date;
    // insert sms into the database
    if ($sacco_sms_status === 'on') {
      // code...
      insertSMSDB($writeDB, $message, $accountContact, $returned_saccoid);
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
      ...
      || !isset($jsonData->reason)|| empty($jsonData->reason)
        ...
        (empty($jsonData->memberid) ? $response->addMessage("account field must not be empty") : false);
        ...
        (!isset($jsonData->loanproduct) ? $response->addMessage("loanproduct field is mandatory and must be provided") : false);
        (empty($jsonData->loanproduct) ? $response->addMessage("loanproduct field must not be empty") : false);
      ...
      if($rowCount === 0) {
        ...
        $response->setHttpStatusCode(500);
        $response->setSuccess(false);
        $response->addMessage("failed to register loan application");
        $response->send();
        exit;
      }
      ...
      while($row = $query->fetch(PDO::FETCH_ASSOC)):
        ...
        $message = "Hello, your loan application of UGX ".$newAmount." is under review; you will be notified once the loan is processed. Loan ID ".$loanappnumber. ". Date: ".$date;
        // insert sms into the database
        insertSMSDB($writeDB, $message, $accountContact, $returned_saccoid);
        // insert email into the database
        insertEMAILDB($writeDB, $message, $saccoEmail, $returned_saccoid);
