...
if (array_key_exists("disburseid",$_GET)) {
  // get task id from query string
  $disburseid = $_GET['disburseid'];
  //check to see if task id in query string is not empty and is number, if not return json error
  if($disburseid == '' || !is_numeric($disburseid)) {
    $response = new Response();
    $response->setHttpStatusCode(400);
    $response->setSuccess(false);
    $response->addMessage("loan disbursed id cannot be blank or must be numeric");
    $response->send();
    exit;
  }
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
      ...
      while($row = $query->fetch(PDO::FETCH_ASSOC)):
        ...
        $message = "Hello, your loan UGX ".$newAmount." has been disbursed to A/C: ".$AccountNumber." (".$firstname." " .$lastname.") in ".$saccoName.". TxID: ".$desposit_trans_id. ". Date: ".$date. ".\nNew balance: UGX ".$newAccountsBalance;
