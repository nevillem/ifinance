...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
  ...
  try {
    ...
    $query = $writeDB->prepare('SELECT accounts_id,account_name,account_code
       from accounts
      WHERE accounts_id=:accountid AND account_types="bank"
    ...
    $returnData['bankaccount'] = $accountsArray;
    ...
    $response->setHttpStatusCode(500);
    $response->setSuccess(false);
    $response->addMessage("failed to get sacco bank acounts");
    $response->send();
    exit;
  }
  ...
  elseif (empty($_GET)) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
      ...
      try {
        ...
        $query = $writeDB->prepare('SELECT accounts_id,account_name,account_code
           from accounts
          WHERE  account_sacco_id = :saccoid AND account_types="bank"');
        $query->bindParam(':saccoid', $returned_saccoid, PDO::PARAM_STR);
        $query->execute();
        ...
        $returnData = array();
        $returnData['rows_returned'] = $rowCount;
        $returnData['bankaccounts'] = $accountsArray;
        // set up response for successful return
        $response = new Response();
        $response->setHttpStatusCode(200);
        $response->setSuccess(true);
