...
try {
  ...
  if (array_key_exists('paybillid', $_GET)) {
    ...
    elseif($_SERVER['REQUEST_METHOD'] === 'DELETE'){
      try {
        // ADD AUTH TO QUERY
        $query = $writeDB->prepare('DELETE from pay_bill where pay_bill_id =:paybillid
          AND pay_bill_sacco_id  = :saccoid');
        $accountsquery->bindParam(':paybillid', $paybillid, PDO::PARAM_STR);
        ...
        if($rowCount === 0):
                ...
                $response->setHttpStatusCode(404);
                $response->setSuccess(false);
                $response->addMessage("sacco paid bill not found");
                $response->send();
                exit;
        else:
                $response = new Response();
                $response->setHttpStatusCode(200);
                $response->setSuccess(true);
                $response->addMessage("sacco paid bill deleted");
                $response->send();
                exit;
      endif;
      }
        ...
        try {
          ...
          try{
           ...
           if ($status==='paid') {
             ...
             $response->setHttpStatusCode(404);
             $response->setSuccess(false);
             $response->addMessage("This bill is already paid");
             $response->send();
             exit;
           }
