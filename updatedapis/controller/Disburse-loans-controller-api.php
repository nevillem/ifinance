...
elseif(empty($_GET)) {
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
      ...
      $gquery->bindParam(':appid', $_applicationid, PDO::PARAM_INT);
      ...
      if ($totalnumber ===0) {
        ...
        $response->setHttpStatusCode(404);
        $response->setSuccess(false);
        $response->addMessage( "This loan has no guarantors yet");
        $response->send();
        exit;
      }
      ...
      $loanpquery->bindParam(':appid', $_applicationid, PDO::PARAM_INT);
      ...
      if ($number_of_guarantors < $totalnumber) {
        ...
        $response->setSuccess(false);
        $response->addMessage( "This loan does not have the required number of guarantors");
        $response->send();
        exit;
      }
