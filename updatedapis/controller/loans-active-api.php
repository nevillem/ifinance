...
elseif(empty($_GET)) {
  ...
  if($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
      // ADD AUTH TO QUERY
      $query = $readDB->prepare('SELECT * from loan_active, members where loan_active.members_member_id = members.member_id 
	  and loan_active.users_user_id = :userid and loan_active.saccos_sacco_id = :saccoid order by loan_id DESC');
