...
if (array_key_exists("sessionid",$_GET)):
  ...
  /**
   * @OA\DELETE(
   *   path="/irembo_version_control/API/usersessions/{sessionid}", tags={"user-auth"},
  ...
  if($_SERVER['REQUEST_METHOD'] === 'DELETE'):
    ...
    try {
      ...
      $query = $writeDB->prepare('delete from sessions_users where session_id = :sessionid and access_token = :accesstoken');
    ...
    /**
     * @OA\PATCH(
     *   path="/irembo_version_control/API/usersession/{sessionid}", tags={"user-auth"},
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'PATCH'):
    ...
    try {
      ...
      $query = $writeDB->prepare('SELECT sessions_users.session_id as sessionid, sessions_users.users_user_id as userid, access_token, refresh_token, user_status, user_login_attempts, access_token_expiry, refresh_token_expiry from sessions_users, users where users.user_id = sessions_users.users_user_id and sessions_users.session_id = :sessionid and sessions_users.access_token = :accesstoken and sessions_users.refresh_token = :refreshtoken');
      $query->bindParam(':sessionid', $sessionid, PDO::PARAM_INT);
      $query->bindParam(':accesstoken', $accesstoken, PDO::PARAM_STR);
      ...
      $query = $writeDB->prepare('update sessions_users set access_token = :accesstoken, access_token_expiry = date_add(:date, INTERVAL :accesstokenexpiryseconds SECOND), refresh_token = :refreshtoken, refresh_token_expiry = date_add(:date2, INTERVAL :refreshtokenexpiryseconds SECOND) where session_id = :sessionid and users_user_id = :userid and access_token = :returnedaccesstoken and refresh_token = :returnedrefreshtoken');
      // bind the user id
...
elseif(empty($_GET)):
  /**
   * @OA\POST(
   *   path="/irembo_version_control/API/usersession", tags={"user-auth"},
   *    summary = "Generate a session, for the EndPoint",
   *  @OA\RequestBody(
   *     @OA\MediaType(
    *         mediaType="application/json",
    *     @OA\Schema(
    *       @OA\Property(property="username", type="string", example="10011001"),
    *       @OA\Property(property="password", type="string", example="45464"),
  *         )
  *       )
  *     ),
  ...
  try {
    ...
    // create db query
    $query = $writeDB->prepare('SELECT user_id, user_role, user_fullname, user_email,
       user_password, user_status, user_login_attempts from users where user_email = :username');
    ...
    $query = $writeDB->prepare('insert into user_activity (sacco_activity_ip, sacco_activity_OS, users_user_id) values (:ip, :os, :userid)');
  ...
  try {
    ...
    $query = $writeDB->prepare('insert into sessions_users (users_user_id, access_token, access_token_expiry, refresh_token, refresh_token_expiry) values (:userid, :accesstoken, date_add(:date2, INTERVAL :accesstokenexpiryseconds SECOND), :refreshtoken, date_add(:date, INTERVAL :refreshtokenexpiryseconds SECOND))');
    // bind the user id
    $query->bindParam(':userid', $returned_id, PDO::PARAM_INT);
    // bind the access token
    $query->bindParam(':accesstoken', $accesstoken, PDO::PARAM_STR);
    ...
    $returnData['user_role'] = $returned_role;
