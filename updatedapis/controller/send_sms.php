...
try {
  ...
  exit;
      ...
      $status = 'N';
      $query = $writeDB->prepare('select * from sms where status = :status');
      ...
      if($rowCount > 0) {
        ...
        foreach ($messages as $message) {
          $send_sms = SendSMS($message['contact'],$message['message']);
      ...
      if (!empty($sent_successfully)) {
        try{
            ...
            foreach ($sent_successfully as $sent_message) {
                $updateSentMessage = $writeDB->prepare('UPDATE sms SET status="Y" WHERE id=:id');
