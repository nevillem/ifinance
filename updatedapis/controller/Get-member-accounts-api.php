...
if($memberid == '' || !is_numeric($memberid)) {
  ...
  $response->setSuccess(false);
  $response->addMessage("member ID cannot be blank or must be numeric");
  $response->send();
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
  ...
  try {
    // create db query
    $query = $writeDB->prepare('SELECT member_accounts_id,account_name,account_code
      FROM accounts,member_accounts
      WHERE member_accounts_member_id=:memberid
    ...
    $response->setHttpStatusCode(500);
    $response->setSuccess(false);
    $response->addMessage("failed to get member acounts $ex");
    $response->send();
    exit;
  }
