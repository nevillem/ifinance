...
try {
  ...
  if (array_key_exists('groupaccountsid', $_GET)) {

    $groupaccountsid = $_GET['groupaccountsid'];

    if($groupaccountsid == '' || !is_numeric($groupaccountsid)) {
      $response = new Response();
      $response->setHttpStatusCode(400);
      $response->setSuccess(false);
      $response->addMessage("filegroup accounts ID cannot be blank or must be numeric");
      $response->send();
      exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
      ...
      try {
        // create db query
        $accountsquery = $writeDB->prepare('SELECT member_accounts_id, member_fname,  member_lname,
          ...
          AND accounts_id=member_accounts_account_id AND member_type="group"');
        $accountsquery->bindParam(':member_accounts_id', $groupaccountsid, PDO::PARAM_STR);
        ...
        if($rowCount === 0) {
          ...
          $response->setSuccess(false);
          $response->addMessage("group account id not found");
          $response->send();
          exit;
        }
        $accountsArray=array();
        while($rowaccount = $accountsquery->fetch(PDO::FETCH_ASSOC)) {
          ...
          $accounts = array(
            ...
            "member_fname" => $member_fname.' '.$member_lname,
        ...
        $response->setHttpStatusCode(500);
        $response->setSuccess(false);
        $response->addMessage("failed to get group acounts");
        $response->send();
        exit;
      }
      ...
      try {
        ...
        $query->bindParam(':memberaccountsid', $groupaccountsid, PDO::PARAM_STR);
        ...
        if($rowCount === 0):
                ...
                $response->setHttpStatusCode(404);
                $response->setSuccess(false);
                $response->addMessage("group account not found");
                $response->send();
                exit;
        else:
                $response = new Response();
                $response->setHttpStatusCode(200);
                $response->setSuccess(true);
                $response->addMessage("group account deleted");
                $response->send();
                exit;
      endif;
      }
        ...
        $response->setHttpStatusCode(500);
        $response->setSuccess(false);
        $response->addMessage("Failed to delete group account - Attached Info");
        $response->send();
        exit;
      }
    ...
    try{
      ...
      if($rowCount === 0):
        ...
        $response->setHttpStatusCode(404);
        $response->setSuccess(false);
        $response->addMessage("No group acount found to update");
        $response->send();
        exit;
      endif;
      ...
      $response->setHttpStatusCode(200);
      $response->setSuccess(true);
      $response->addMessage("group account has been updated");
      $response->setData($returnData);
      $response->send();
      exit;
    ...
    // if error with sql query return a json error
      ...
      $response->setHttpStatusCode(500);
      $response->setSuccess(false);
      $response->addMessage("Failed to update group accounts - check your data for errors" . $ex);
      $response->send();
      exit;
    }
    }
   ...
   elseif(empty($_GET)){
        ...
        try {
          $query = $writeDB->prepare('SELECT member_id, member_fname, member_mname, member_lname,
            member_contact, member_gender, member_email,member_address,member_identification,members_account_number
             from members where saccos_sacco_id =:saccoid AND member_type="group"');
          $query->bindParam(':saccoid', $returned_saccoid, PDO::PARAM_INT);
          $query->execute();
          ...
          $accountsquery = $writeDB->prepare('SELECT accounts_id,account_name,account_code from accounts,member_accounts
            WHERE member_accounts_sacco_id = :saccoid AND member_accounts_member_id =:memberid AND accounts_id=member_accounts_account_id ');
    ...
    elseif($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        try {
          ...
          if(!isset($jsonData->group_id) || !isset($jsonData->accounts_attached)|| !isset($jsonData->doj)) {
            $response = new Response();
            $response->setHttpStatusCode(400);
            $response->setSuccess(false);
            (!isset($jsonData->group_id) ? $response->addMessage("group is mandatory and must be provided") : false);
            (empty($jsonData->group_id) ? $response->addMessage("group field must not be empty") : false);
          ...
          try{
            ...
            $query->bindParam(':id', $jsonData->group_id, PDO::PARAM_STR);
            $query->execute();
            ...
            if ($rowCount === 0) {
              ...
              $response->setHttpStatusCode(500);
              $response->setSuccess(false);
              $response->addMessage("group with an id $jsonData->group_id does not exist");
              $response->send();
              exit;
            }
            for ($i=0; $i < count($jsonData->accounts_attached); $i++) {
              $countsatt=$jsonData->accounts_attached[$i];
              $query = $writeDB->prepare('SELECT * from member_accounts
                where member_accounts_account_id = :memberaccounts AND  member_accounts_member_id=:groupid');
                $query->bindParam(':memberaccounts', $countsatt, PDO::PARAM_STR);
              $query->bindParam(':groupid', $jsonData->group_id, PDO::PARAM_STR);
              $query->execute();
              ...
              if ($rowCount > 0) {
                ...
                $response->setHttpStatusCode(500);
                $response->setSuccess(false);
                $response->addMessage("some of the accounts already exists for this group");
                $response->send();
                exit;
              }
              ...
              $query->bindParam(':member_id', $jsonData->group_id, PDO::PARAM_STR);
            ...
            $memberAccountsArray = array();
              ...
              $returnData = array();
              $returnData['rows_returned'] = $rowCount;
              $returnData['groupaccountaccount'] = $memberAccountsArray;
