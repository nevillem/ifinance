    ...
    try {
      ...
      if ($account_status !== 'active') {
        ...
        if ($AccountBalance < $AccountCheckBalance) {
          ...
          $response->setHttpStatusCode(400);
          $response->setSuccess(false);
          $response->addMessage("Not enough funds on group account to make transaction");
          $response->send();
          exit;
        }
      ...
      while($row = $query->fetch(PDO::FETCH_ASSOC)):
        ...
        $depositmessage = "A saving of UGX ".$newAmount." has been made to group A/C: ".$deposit_AccountNumber." (".$deposit_firstname.") in ".$saccoName." has been made from A/C: ".$AccountNumber." (".$firstname." " .$lastname.")"." TxID: ".$deposit_transactionID. ". Date: ".$date. ".\nNew balance: UGX ".$deposit_newAccountBalance;
