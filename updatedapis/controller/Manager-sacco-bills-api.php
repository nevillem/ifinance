...
try {
  ...
  if (array_key_exists('billid', $_GET)) {
    ...
    elseif($_SERVER['REQUEST_METHOD'] === 'DELETE'){
      try {
        // ADD AUTH TO QUERY
        $query = $writeDB->prepare('DELETE from bills_tb where bill_id =:billid AND bill_saccoid  = :saccoid');
        $accountsquery->bindParam(':billid', $billid, PDO::PARAM_STR);
        ...
        if($rowCount === 0):
                ...
                $response->setHttpStatusCode(404);
                $response->setSuccess(false);
                $response->addMessage("sacco bill not found");
                $response->send();
                exit;
        else:
                $response = new Response();
                $response->setHttpStatusCode(200);
                $response->setSuccess(true);
                $response->addMessage("sacco bill deleted");
                $response->send();
                exit;
      endif;
      }
        ...
        $response->setHttpStatusCode(500);
        $response->setSuccess(false);
        $response->addMessage("Failed to delete sacco bill - Attached Info");
        $response->send();
        exit;
      }
    ...
    try{
      ...
      $billnumberUpdate = false;
      $vendorUpdate = false;
      $accountUpdate = false;
      $amountUpdate = false;
      $duedateUpdate = false;
      $transdateUpdate = false;
      $notesUpdate = false;
      // create blank query fields string to append each field to
      $queryFields = "";

      // check if name exists in PATCH
      if(isset($jsonData->billnumber)):
        // set title field updated to true
        $billnumberUpdate = true;
        // add name field to query field string
        $queryFields .= "bill_number   = :billnumber, ";
      endif;
      if(isset($jsonData->vendor)):
        // set title field updated to true
        $vendorUpdate = true;
        // add name field to query field string
        $queryFields .= "bill_vendor_id   = :vendor, ";
      endif;
      // check if address exists in PATCH
      if(isset($jsonData->expenseaccount)):
        // set Address field updated to true
        $accountUpdate = true;
        // add address field to query field string
        $queryFields .= "expense_account_id   = :expenseaccount, ";
      endif;
      if(isset($jsonData->amount)):
        // set Address field updated to true
        $amountUpdate = true;
        // add address field to query field string
        $queryFields .= "bill_amount  = :amount, ";
      endif;
      if(isset($jsonData->duedate)):
        // set Address field updated to true
        $duedateUpdate = true;
        // add address field to query field string
        $queryFields .= "duedate  = :duedate, ";
      endif;
      if(isset($jsonData->transdate)):
        // set Address field updated to true
        $transdateUpdate = true;
        // add address field to query field string
        $queryFields .= "transactiondate  = :transdate, ";
      endif;
      if(isset($jsonData->notes)):
        // set Address field updated to true
        $notesUpdate = true;
        // add address field to query field string
        $queryFields .= "bill_notes  = :notes, ";
      endif;
      // remove the right hand comma and trailing space
      $queryFields = rtrim($queryFields, ", ");

      // check if any branch fields supplied in JSON
      if($billnumberUpdate === false && $vendorUpdate === false && $accountUpdate === false && $amountUpdate === false
      && $duedate === false && $transdateUpdate === false && $notesUpdate === false):
        $response = new Response();
        $response->setHttpStatusCode(400);
        $response->setSuccess(false);
      ...
      $query = $writeDB->prepare('SELECT * from bills_tb where bill_id =:billid
        AND vendors_saccoid = :saccoid');
      ...
      if($rowCount === 0):
        ...
        $response->setHttpStatusCode(404);
        $response->setSuccess(false);
        $response->addMessage("No sacco bill found to update");
        $response->send();
        exit;
      endif;
      // create the query string including any query fields
      $queryString = "UPDATE bills_tb set ".$queryFields." where bill_id  = :id";
      // prepare the query
      $query = $writeDB->prepare($queryString);

      // if name has been provided
      if($billnumberUpdate === true):
        // bind the parameter of the new value from the object to the query (prevents SQL injection)
        $query->bindParam(':billnumber', $jsonData->billnumber, PDO::PARAM_STR);
      endif;
      if($vendorUpdate === true):
        // bind the parameter of the new value from the object to the query (prevents SQL injection)
        $query->bindParam(':vendor', $jsonData->vendor, PDO::PARAM_STR);
      endif;
      // if name has been provided
      if($accountUpdate === true):
        // bind the parameter of the new value from the object to the query (prevents SQL injection)
        $query->bindParam(':expenseaccount', $jsonData->expenseaccount, PDO::PARAM_STR);
      endif;
      // if name has been provided
      if($amountUpdate === true):
        // bind the parameter of the new value from the object to the query (prevents SQL injection)
        $query->bindParam(':amount', $jsonData->amount, PDO::PARAM_STR);
      endif;
      // if name has been provided
      if($duedateUpdate === true):
        // bind the parameter of the new value from the object to the query (prevents SQL injection)
        $query->bindParam(':duedate', $jsonData->duedate, PDO::PARAM_STR);
      endif;
      // if name has been provided
      if($transdateUpdate === true):
        // bind the parameter of the new value from the object to the query (prevents SQL injection)
        $query->bindParam(':transdate', $jsonData->transdate, PDO::PARAM_STR);
      endif;
      // if name has been provided
      if($notesUpdate === true):
        // bind the parameter of the new value from the object to the query (prevents SQL injection)
        $query->bindParam(':notes', $jsonData->notes, PDO::PARAM_STR);
      endif;
      $query->bindParam(':id', $billid, PDO::PARAM_STR);
      $query->execute();
      if($rowCount === 0):
        // set up response for unsuccessful return
        $response = new Response();
      ...
      $response->setHttpStatusCode(200);
      $response->setSuccess(true);
      $response->addMessage("bill has been updated");
      $response->setData($returnData);
      $response->send();
      exit;
    ...
    // if error with sql query return a json error
      ...
      $response->setHttpStatusCode(500);
      $response->setSuccess(false);
      $response->addMessage("Failed to update bill - check your data for errors" . $ex);
      $response->send();
      exit;
    }
    }
