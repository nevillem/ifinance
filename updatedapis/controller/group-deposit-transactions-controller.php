  ...
  if($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
      ...
      $query = $readDB->prepare('SELECT * from desposit_transactions, members,member_accounts, users,accounts
        ...
        AND member_accounts.member_accounts_id = desposit_transactions.deposit_account_member_id   AND
        members.member_id=member_accounts.member_accounts_member_id
        AND deposit_id = :transactionid
...
elseif(empty($_GET)) {
  ...
  if($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
      // ADD AUTH TO QUERY
      $query = $readDB->prepare('SELECT * from desposit_transactions, members,member_accounts,accounts
        WHERE desposit_transactions.members_member_id = members.member_id
        AND desposit_transactions.users_user_id = :userid
        AND member_type="group"
        AND accounts.accounts_id=member_accounts.member_accounts_account_id
        AND accounts.account_types="saving"
        AND desposit_transactions.members_member_id = member_accounts.member_accounts_member_id
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
      ...
      if(!isset($jsonData->account)|| !isset($jsonData->group_account) || !isset($jsonData->amount)
      || !isset($jsonData->mop)|| !isset($jsonData->dop)|| !isset($jsonData->pincode)
          || !isset($jsonData->deposited) || empty($jsonData->account) || empty($jsonData->group_account) || empty($jsonData->amount)
          || empty($jsonData->deposited) || empty($jsonData->pincode)) {
        $response = new Response();
        $response->setHttpStatusCode(400);
        $response->setSuccess(false);
        (!isset($jsonData->account) ? $response->addMessage("group field is mandatory and must be provided") : false);
        (!isset($jsonData->group_account) ? $response->addMessage("group account field is mandatory and must be provided") : false);
        ...
        (empty($jsonData->account) ? $response->addMessage("group account field must not be empty") : false);
        (empty($jsonData->group_account) ? $response->addMessage("group account field must not empty") : false);
      ...
      $member_account = $jsonData->group_account;
      ...
      $query = $writeDB->prepare('select * from desposit_transactions,users where desposit_transactions.users_user_id = users.user_id and  deposit_id = :id');
      ...
      while($row = $query->fetch(PDO::FETCH_ASSOC)):
        ...
        $message = "A saving of UGX ".$newAmount." has been made to group A/C: ".$AccountNumber." (".$firstname." " .$lastname.") in ".$saccoName.". TxID: ".$desposit_trans_id. ". Date: ".$date. ".\nNew balance: UGX ".$newAccountsBalance;
