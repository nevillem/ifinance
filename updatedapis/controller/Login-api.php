...
/**
 * @OA\Info(
 * title="Irembo Finance Platform",
 * version="v2",
 * description="Irembo Finance API Documentation Response Entries",
 * @OA\Contact(name="Ahuriire Uganda LTD", url="https://ahuriire.com/", email="tech@ahuriire.com"),
 * @OA\License(name="ahuriire license", url = "https://ahuriire.com/license")
 *)
 */
...
if (array_key_exists("sessionid",$_GET)):
  ...
  /**
   * @OA\DELETE(
   *   path="/irembo_version_control/API/sessions/{sessionid}", tags={"auth-sacco"},
   *    summary ="Delete Sessions - EndPoint",
   *     @OA\MediaType(
   ...
   *   ),
   *     @OA\Response(response="201", description="Session has been deleted"),
  ...
  if($_SERVER['REQUEST_METHOD'] === 'DELETE'):
     ...
     * @OA\PATCH(
     *   path="/irembo_version_control/API/sessions/{sessionid}", tags={"auth-sacco"},
     *    summary = "Refresh the user to stay logged in - interval 5s",
     *   @OA\Parameter(
      *      name="authorization_token",
      *     in="header",
      *     required=true,
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'PATCH'):
    ...
    try {
      ...
      $query = $writeDB->prepare('SELECT sessions.session_id as sessionid, sessions.saccos_sacco_id as saccoid, access_token, refresh_token, sacco_status, sacco_login_attempts, access_token_expiry, refresh_token_expiry from sessions, saccos where saccos.sacco_id = sessions.saccos_sacco_id and sessions.session_id = :sessionid and sessions.access_token = :accesstoken and sessions.refresh_token = :refreshtoken');
      $query->bindParam(':sessionid', $sessionid, PDO::PARAM_INT);
      $query->bindParam(':accesstoken', $accesstoken, PDO::PARAM_STR);
...
elseif(empty($_GET)):
  /**
   * @OA\POST(
   *   path="/irembo_version_control/API/sessions", tags={"auth-sacco"},
   *    summary = "Generate a session, for the EndPoint",
   *  @OA\RequestBody(
   *     @OA\MediaType(
    *         mediaType="application/json",
    *     @OA\Schema(
    *       @OA\Property(property="username", type="string", example="ibigega23@gmail.com"),
    *       @OA\Property(property="password", type="string", example="Newcastle789@"),
  *         )
  *       )
  *     ),
   *     @OA\Response(response="201", description="Session has been started"),
   *     @OA\Response(response="400", description="You have received a bad response"),
   *     @OA\Response(response="401", description="unauthorised please try again"),
   *     @OA\Response(response="404", description="Not found"),
   *     @OA\Response(response="500", description="An internal server error")
  ...
  // attempt to query the database to check sacco details - use write connection as it needs to be synchronous for password/token
  try {
    $username = $jsonData->username;
    $password = $jsonData->password;
    // create db query
    $query = $writeDB->prepare('SELECT sacco_id, sacco_name, sacco_email, sacco_password, sacco_status, sacco_login_attempts from saccos where sacco_email = :username');
    ...
    // save returned details into variables
    $returned_id = $row['sacco_id'];
    $returned_fullname = $row['sacco_name'];
    $returned_username = $row['sacco_email'];
    $returned_password = $row['sacco_password'];
    $returned_active = $row['sacco_status'];
    $returned_loginattempts = $row['sacco_login_attempts'];
    ...
    if(!password_verify($password, $returned_password)):
      ...
      $query = $writeDB->prepare('update saccos set sacco_login_attempts = sacco_login_attempts+1 where sacco_id = :id');
      // bind the sacco id
    ...
    //register a  log into the Database to capture your IP and Mac Address caputred at login to prevent fraud
    $ipaddress = $_SERVER['REMOTE_ADDR'];
    $os = $_SERVER["HTTP_USER_AGENT"];
    $query = $writeDB->prepare('insert into sacco_activity (sacco_activity_ip, sacco_activity_OS, saccos_sacco_id) values (:ip, :os, :saccoid)');
    $query->bindParam(':ip', $ipaddress, PDO::PARAM_STR);
    $query->bindParam(':os', $os, PDO::PARAM_STR);
    $query->bindParam(':saccoid', $returned_id, PDO::PARAM_INT);
    $query->execute();
  }
  catch(PDOException $ex) {
    $response = new Response();
  ...
  try {
    ...
    $query = $writeDB->prepare('update saccos set sacco_login_attempts = 0 where sacco_id = :id');
    // bind the sacco id
    $query->bindParam(':id', $returned_id, PDO::PARAM_INT);
    // run the query
    $query->execute();
    ...
    $query = $writeDB->prepare('insert into sessions (saccos_sacco_id, access_token, access_token_expiry, refresh_token, refresh_token_expiry) values (:saccoid, :accesstoken, date_add(:date, INTERVAL :accesstokenexpiryseconds SECOND), :refreshtoken, date_add(:date2, INTERVAL :refreshtokenexpiryseconds SECOND))');
    // bind the sacco id
    $query->bindParam(':saccoid', $returned_id, PDO::PARAM_INT);
    // bind the access token
    $query->bindParam(':accesstoken', $accesstoken, PDO::PARAM_STR);
